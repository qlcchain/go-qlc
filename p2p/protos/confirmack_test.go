...
func TestConfirmAckBlockPacket(t *testing.T) {
	...
	if err = json.Unmarshal([]byte(testBlockConfirmack1), &blk1); err != nil {
		t.Fatal(err)
	}

	if err = json.Unmarshal([]byte(testBlockConfirmack2), &blk2); err != nil {
		t.Fatal(err)
	}
	...
	if blk1.GetHash() != ack.Hash[0] {
		t.Fatal("hash error")
	}

	if blk2.GetHash() != ack.Hash[1] {
		t.Fatal("hash error")
	}
