...
func updateMemory(vm *C.struct_VirtualMachine) {
	...
	managedVM.Memory.Memory = *(*[]byte)(unsafe.Pointer(&memorySlice))
}
...
func FullAOTCompile(vm *exec.VirtualMachine) *AOTContext {
	...
	nativeVM := C.vm_alloc()
	C.vm_build(nativeVM, C.uintptr_t(uintptr(unsafe.Pointer(vm))), C.uint64_t(len(vm.Memory.Memory)))
	if len(vm.Memory.Memory) > 0 {
		C.memcpy(unsafe.Pointer(nativeVM.mem), unsafe.Pointer(&vm.Memory.Memory[0]), C.ulong(len(vm.Memory.Memory)))
	}
