...
func (z *TokenMeta) DecodeMsg(dc *msgp.Reader) (err error) {
	...
	for zb0001 > 0 {
		...
		case "rep":
			err = dc.ReadExtension(&z.RepBlock)
			if err != nil {
				return
			}
		case "open":
...
func (z *TokenMeta) EncodeMsg(en *msgp.Writer) (err error) {
	...
	err = en.WriteExtension(&z.RepBlock)
	if err != nil {
		return
	}
	// write "open"
...
func (z *TokenMeta) MarshalMsg(b []byte) (o []byte, err error) {
	...
	o, err = msgp.AppendExtension(o, &z.RepBlock)
	if err != nil {
		return
	}
	// string "open"
...
func (z *TokenMeta) UnmarshalMsg(bts []byte) (o []byte, err error) {
	...
	for zb0001 > 0 {
		...
		case "rep":
			bts, err = msgp.ReadExtensionBytes(bts, &z.RepBlock)
			if err != nil {
				return
			}
		case "open":
...
func (z *TokenMeta) Msgsize() (s int) {
	s = 1 + 5 + msgp.ExtensionPrefixSize + z.Type.Len() + 7 + msgp.ExtensionPrefixSize + z.Header.Len() + 4 + msgp.ExtensionPrefixSize + z.RepBlock.Len() + 5 + msgp.ExtensionPrefixSize + z.OpenBlock.Len() + 8 + msgp.ExtensionPrefixSize + z.Balance.Len()
	return
}
